...
CapSense cs_4_2 = CapSense(140,2);        // 10M resistor between pins 4 & 2, pin 2 is sensor pin, add a wire and or foil if desired
...
void loop()                    
{
  ...
  cs_4_2.cSenseCalibrateTouch();
  delay(1);
    long total1 = 0;
    long start = millis();
   // while(millis() < start + 5){
     total1 +=  cs_4_2.cSensePin(400);
